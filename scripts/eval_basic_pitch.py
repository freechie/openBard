#!/usr/bin/env python3
"""Evaluate Basic Pitch on isolated-piano.wav.

Reports pitch match rate within 100ms, long spurious notes, latency, and memory.
"""
import json
import sys
import time
...
def main() -> None:
    ...
    # Lazy import so a missing package prints a clear error instead of crashing at module load.
    try:
        from basic_pitch.inference import predict
        from basic_pitch import ICASSP_2022_MODEL_PATH
        import os
        # TFLite is the backend that loads with this TensorFlow pin.
    ...
    if results["spurious_long_notes"]:
        print(f"WARN: Spurious long notes detected ({len(results['spurious_long_notes'])}):")
    ...
    if results["missed_notes"]:
        print(f"WARN: Missed notes ({len(results['missed_notes'])}):")
    ...
    print(f"  {'OK' if all_notes_detected else 'FAIL'} All expected chord pitches detected within 100ms")
    print(f"  {'OK' if no_long_spurious else 'FAIL'} No long spurious notes")
    print()
